...
rfc = RandomForestClassifier(n_estimators=100, max_depth=None, random_state=0, bootstrap=True)
rfc.fit(X_trainVP, y_trainVP)

#array di decision tree (componenenti del RandomForest rfc) basati sul training set vero e proprio
T = np.empty((rfc.n_estimators), dtype = object)
for i in range (0, rfc.n_estimators):
    T[i] = rfc[i]
...
rfc1 = RandomForestClassifier(n_estimators=100, max_depth=None, random_state=0, bootstrap=True)
rfc1.fit(X_val, y_pred)

#array di decision tree (componenenti del RandomForest rfc1) basati su y_pred
P = np.empty_like(T)
for i in range (0, rfc1.n_estimators):
    P[i] = rfc1[i]
...
def new_prediction(sample):
    ...
    for i in range(0, rfc.n_classes_):
...
c = np.empty((rfc.n_estimators), dtype = int)
p = np.empty_like(c)

countc = np.empty((rfc.n_classes_), dtype = int)
...
#implementazione tramite RandomForestClassifier senza l'implementazione del surprisingly popular algorithm
rfc3 = RandomForestClassifier(n_estimators=100, max_depth=None, random_state=0, bootstrap=True)

#calcolo l'accuratezza tramite cross-validation su tutto il dataset
from sklearn.model_selection import cross_val_score
scores = cross_val_score(rfc3, X, y, cv=5)
acc_RF_single = scores.mean()


print("Accuratezza RandomForest tramite SPA: {:.4%}".format(acc_RF_SPA))
print("Accuratezza RandomForest non-SPA: {:.4%}".format(acc_RF_single))
